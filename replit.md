...
- Bard: Straight-line jumping movement with activation and forced swap mechanics:
  - Movement: Can single-step to adjacent nodes OR jump over pieces in a straight line -起跳点、跳过的棋子和落点必須在同一直線上 (starting point, jumped piece, and landing point must be collinear); can chain multiple jumps in the same direction
