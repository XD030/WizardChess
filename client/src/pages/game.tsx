...
export default function Game() {
  ...
  const playModeRef = useRef<PlayMode>("pvp");
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      if (playModeRef.current === "solo") return;
  ...
  function handleRestartGame() {
    ...
    const newReady: ReadyState =
      playModeRef.current === "solo" ? { white: true, black: true } : { white: false, black: false };
    ...
    const initial: SyncedState = {
      ...
      startingMode: "manual",
      ready: newReady,
      gameStarted: playModeRef.current === "solo",
      pendingGuard: null,
    };
    ...
    setReady(newReady);
    setGameStarted(playModeRef.current === "solo");
  ...
  const handleGuardConfirm = () => {
    ...
    if (playModeRef.current === "solo") applySyncedState(syncState);
    else broadcastState(syncState);
  };
