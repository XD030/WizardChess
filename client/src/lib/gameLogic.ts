...
// Key: Exclude SQUARES (same-color diagonals) but allow PARALLELOGRAMS (different-color diagonals)
      ...
      if (adjacency[adj1Idx].includes(adj2Idx)) {
        ...
        if (targetIdx !== -1) {
          ...
          if (adjacency[adj1Idx].includes(targetIdx) && adjacency[adj2Idx].includes(targetIdx)) {
            ...
            // CRITICAL: Only allow moves where opposite corners have DIFFERENT colors
            // This ensures we only move along parallelogram diagonals, not square diagonals
            // Same color diagonal = SQUARE (not allowed)
            // Different color diagonal = PARALLELOGRAM (allowed)
            const targetIsBlack = isBlackTriangle(targetNode.row, targetNode.col);
            if (currentIsBlack === targetIsBlack) {
              // Same color = square diagonal, skip
              continue;
            }
