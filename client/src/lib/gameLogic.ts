...
export function calculateRangerMoves(
  ...
  for (const adjIdx of adjacency[nodeIdx]) {
    ...
    console.log(`游侠炮攻击检查: from(${piece.row},${piece.col}) -> adj(${adjNode.row},${adjNode.col}), pieceAtAdj=${pieceAtAdj}`);
    ...
    if (pieceAtAdj !== -1) {
      ...
      while (nextIdx !== -1) {
        ...
        if (pieceAtNext !== -1) {
          ...
          if (targetPiece.side !== piece.side && targetPiece.side !== 'neutral') {
            console.log(`    ✓ 可以炮攻击位置(${nextNode.row},${nextNode.col})`);
